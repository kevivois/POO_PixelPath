...
class Road(x:Int,y:Int,tileset:TiledMapTileSet,var layer:TiledMapTileLayer,car_speed:Double=1.5) {
  ...
  def init_car(speed:Double):Unit = {
    var rdm = Random.between(0,2)
    var car:SimpleCar = new SimpleCar(0,y+1,speed,1)
    cars.addOne(car)
    if(rdm == 1){
      var car:SimpleCar = new SimpleCar(layer.getWidth-1,y+2,speed,-1)
      cars.addOne(car)
    }
  }
  ...
  def drawCars(g: GdxGraphics):Unit = {
    if(is_started){
      for(car <- cars){
        println(s"position of car ${cars.indexOf(car)} = "+car.getPosition)
        if(!car.isMoving){
          if(car.direction == 1){
